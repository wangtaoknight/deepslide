...
def process_xml_files(files_to_process, output_path):
    ...
    for file_path in files_to_process:
        if os.path.basename(file_path).split('_output')[0] in existing_files:
            print('File {} already exists!'.format(file_path)) #判断文件是否已经处理
        else:
            print('Processing file {}'.format(file_path))      # 如果没有经过处理，则进入处理。每次处理完生成文件 _output
            ...
            for i in root.iter('Annotation'):    # 将annotation修改为Snnotations 加了一个s
